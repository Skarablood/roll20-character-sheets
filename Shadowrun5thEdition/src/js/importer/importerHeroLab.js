...
const importerHeroLabProcess = mancerValues => {
  ...
  const characterHeroLab = convertStatBlockArrayToObject(Object.assign({}, splitStatBlock))

  console.log("Hero Lab Data")
  console.log(characterHeroLab)
}

const convertStatBlockArrayToObject = statBlockObject => {
  ...
  const loopThroughStatBlockObject = (key, findString) => {
  ...
  let statBlockValuesArray = Object.values(statBlockObject);
  //Set character keys that are individual or a list
  //Example "Armor: 7" or "Limits: Physical 11, Mental 5, Social 6"
  for (let [key, findString] of Object.entries(characterHeroLab)) {
    if (key != 'name' && key != 'special' && key != 'contacts') {
      if (findStringInArray(statBlockValuesArray, findString)) {
        loopThroughStatBlockObject(key, findString)
      } else {
  ...
  const findContacts = () => {
    contacts = []
    for (let [indexKey, value] of Object.entries(statBlockObject)) {
      if (value.includes('(Connection ') && value.includes(', Loyalty ')) {
        contacts.push(value)
        delete statBlockObject[indexKey]
      }
    }
  ...
  characterHeroLab.contacts = findContacts()

  //Turn special nested values into a list. Example:
  //12: "Augmentations:"
  //13: "   Datajack (Alphaware)"
  characterHeroLab.special.forEach(specialElement => {
    const keyName = specialElement.toLowerCase()
